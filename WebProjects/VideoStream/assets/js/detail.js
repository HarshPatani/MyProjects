...
fetch(
  "https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics%2Cplayer&id=" +
    video_id +
    "&key=AIzaSyBU-6SJD9pwZHrKMGbDNIwbtVHVJPzwPjk"
)
  .then((res) => {
    return res.json();
  })
  .then((data) => {
    ...
    document.getElementById("player_iframe").src =
...
function getSuggestions() {
  fetch(
    "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=15&relatedToVideoId=" +
      video_id +
      "&type=video&key=AIzaSyAdWaVR_CQXV4SELrqkK6_qbtvsepjE03o"
  )
    .then((res) => {
      return res.json();
    })
    .then((data) => {
      ...
      const suggestions_container = document.getElementById(
      ...
      for (let i = 0; i < suggestion_list.length; i++) {
        ...
        const suggestion_video_thumbnail =
          single_suggestion.snippet.thumbnails.high.url;
